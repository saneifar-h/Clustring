...
{
    [TestClass]
    public class AsyncThreadClusteringTest
    {
        private AsyncClusterManager asyncClusterManager;
        ...
        public void Test_Async_Cluster_Simple_Allocation_JustFor_SameKey()
        {
            ...
            st.Start();
            Task.Run(() => asyncClusterManager.Cluster(item1));
            Task.Run(() => asyncClusterManager.Cluster(item2));
            Task.Run(() => asyncClusterManager.Cluster(item3));
        ...
        public void Test_Async_Cluster_Simple_Allocation_JustFor_DifferentKey()
        {
            ...
            Assert.IsTrue(logger.LstEnqueue.Select(i => i.ClusterInfo.Id).Distinct().Count() > 1);
            Console.WriteLine($"Avg Process {lstDuration.Average()}");
        }
        ...
        public void Test_Async_Cluster_Complex_Allocation_JustFor_DifferentKey()
        {
            ...
            for (var i = 0; i < itemNumber; i++)
            {
                ...
                tasks.Add(new Task(() => { asyncClusterManager.Cluster(haveClusterKeyItem); }));
            }
        ...
        public void Test_Async_Cluster_Complex_Allocation_JustFor_Pause_And_Start()
        {
            ...
            asyncClusterManager.Pause();
            foreach (var task in tasks) task.Start();

            Task.Run(() =>
            {
                Task.Delay(2000).Wait();
                asyncClusterManager.Resume();
            });
